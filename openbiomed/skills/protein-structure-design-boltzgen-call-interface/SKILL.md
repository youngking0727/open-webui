---
name: protein-structure-design-boltzgen-call-interface
description: >
  Call interface for OpenBioMed protein structure prediction (protein_folding task, e.g. ESMFold).
...
Call protein structure prediction via the `/run_pipeline/` endpoint of an OpenBioMed service with `task="protein_folding"`.
