---
name: protein-subcellular-localization-prediction-biot5-call-interface
description: >
  Call interface for OpenBioMed protein subcellular localization prediction (BioT5, protein_question_answering task).
...
Predict subcellular localization for proteins from their amino acid sequences via the `/run_pipeline/` endpoint of an OpenBioMed service with `task="protein_question_answering"` and `model="biot5"`.
